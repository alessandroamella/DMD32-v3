/*--------------------------------------------------------------------------------------
 ...
 DMD32.cpp - Function and support library for the Freetronics DMD, a 512 LED
...
ESP32 Modified by: Khudhur Alfarhan  // Qudoren@gmail.com 1/Oct./2020
 ...
 Note that the DMD library uses the SPI port  (VSPI) for the fastest, low
overhead writing to the display. Keep an eye on conflicts if there are any other
...
/*--------------------------------------------------------------------------------------
 ...
 Note this currently uses the SPI port for the fastest performance to the DMD,
be careful of possible conflicts with other SPI port devices
--------------------------------------------------------------------------------------*/
DMD::DMD(byte panelsWide, byte panelsHigh, uint8_t pin_a, uint8_t pin_b,
         uint8_t pin_sclk, uint8_t pin_noe, uint8_t pin_spi_clk,
         uint8_t pin_spi_mosi, uint8_t pin_other_spi_cs) {
  ...
  PIN_DMD_A = pin_a;
  PIN_DMD_B = pin_b;
  PIN_DMD_SCLK = pin_sclk;
  PIN_DMD_nOE = pin_noe;
  PIN_DMD_CLK = pin_spi_clk;
  PIN_DMD_R_DATA = pin_spi_mosi;
  PIN_OTHER_SPI_nCS = pin_other_spi_cs;
  ...
  // initialize the SPI port with custom pins. This will configure CLK and MOSI
  // pins.
  ...
  // HIGH); pinMode(PIN_DMD_CLK, OUTPUT); pinMode(PIN_DMD_R_DATA, OUTPUT)
...
--------------------------------------------------------------------------------------*/
void DMD::writePixel(unsigned int bX, unsigned int bY, byte bGraphicsMode,
                     byte bPixel) {
  ...
  byte panel =
      (bX / DMD_PIXELS_ACROSS) + (DisplaysWide * (bY / DMD_PIXELS_DOWN));
  ...
  if (amountY == 0 && amountX == -1) {
    ...
    for (int i = 0; i < DMD_RAM_SIZE_BYTES * DisplaysTotal; i++) {
      if ((i % (DisplaysWide * 4)) == (DisplaysWide * 4) - 1) {
        ...
        bDMDScreenRAM[i] =
            (bDMDScreenRAM[i] << 1) + ((bDMDScreenRAM[i + 1] & 0x80) >> 7);
      }
    }
    ...
    for (int i = (DMD_RAM_SIZE_BYTES * DisplaysTotal) - 1; i >= 0; i--) {
      if ((i % (DisplaysWide * 4)) == 0) {
        ...
        bDMDScreenRAM[i] =
            (bDMDScreenRAM[i] >> 1) + ((bDMDScreenRAM[i - 1] & 1) << 7);
      }
    }
...
hardware. Call 4 times to scan the whole display which is made up of 4
interleaved rows within the 16 total rows. Insert the calls to this function
into the main loop for the highest call rate, or from a timer interrupt
--------------------------------------------------------------------------------------*/
void DMD::scanDisplayBySPI() {
  ...
  if (digitalRead(PIN_OTHER_SPI_nCS) == HIGH) {
    ...
    // Optimization: Start the transaction once before the loop
    ...
    // End the transaction after all data is sent
    vspi->endTransaction();
...
int DMD::drawChar(const int bX, const int bY, const unsigned char letter,
  ...
  if (pgm_read_byte(this->Font + FONT_LENGTH) == 0 &&
    ...
    // zero length is flag indicating fixed width font (array does not contain
    // width data entries)
