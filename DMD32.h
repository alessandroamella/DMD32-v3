/*--------------------------------------------------------------------------------------
 ...
 DMD32.h   - Function and support library for the Freetronics DMD, a 512 LED
matrix display panel arranged in a 32 x 16 layout.

This LIibrary (DMD32) is a fork of original DMD library  was modified to work on
ESP32 Modified by: Khudhur Alfarhan  // Qudoren@gmail.com 1/Oct./2020

 Note that the DMD32 library uses the SPI port for the fastest, low overhead
...
// SPI library must be included for the SPI scanning/connection method to the
// DMD
#include <SPI.h>

// ######################################################################################################################
// ######################################################################################################################
// Default ESP32 pins for the display connection (Using VSPI)
// These are provided as defaults but can be overridden in the constructor
#define DMD_DEFAULT_PIN_nOE                                                    \
  22 // D22 active low Output Enable, setting this low lights all the LEDs in
     // the selected rows. Can pwm it at very high frequency for brightness
     // control.
#define DMD_DEFAULT_PIN_A 19      // D19
#define DMD_DEFAULT_PIN_B 21      // D21
#define DMD_DEFAULT_PIN_CLK 18    // D18_SCK  is SPI Clock if SPI is used
#define DMD_DEFAULT_PIN_SCLK 2    // D02
#define DMD_DEFAULT_PIN_R_DATA 23 // D23_MOSI is SPI Master Out if SPI is used
#define DMD_DEFAULT_PIN_OTHER_SPI_nCS                                          \
  SS // Chip select pin for other SPI devices (e.g., Ethernet W5100)
// ######################################################################################################################
// ######################################################################################################################
...
#define DMD_BITSPERPIXEL                                                       \
  1 // 1 bit per pixel, use more bits to allow for pwm screen brightness control
...
class DMD {
public:
  // Instantiate the DMD with custom pin configuration
  DMD(byte panelsWide, byte panelsHigh, uint8_t pin_a = DMD_DEFAULT_PIN_A,
      uint8_t pin_b = DMD_DEFAULT_PIN_B,
      uint8_t pin_sclk = DMD_DEFAULT_PIN_SCLK,
      uint8_t pin_noe = DMD_DEFAULT_PIN_nOE,
      uint8_t pin_spi_clk = DMD_DEFAULT_PIN_CLK,
      uint8_t pin_spi_mosi = DMD_DEFAULT_PIN_R_DATA,
      uint8_t pin_other_spi_cs = DMD_DEFAULT_PIN_OTHER_SPI_nCS);
  // virtual ~DMD();

  // Set or clear a pixel at the x and y location (0,0 is the top left corner)
  void writePixel(unsigned int bX, unsigned int bY, byte bGraphicsMode,
                  byte bPixel);
  ...
  // Scan the dot matrix LED panel display, from the RAM mirror out to the
  // display hardware. Call 4 times to scan the whole display which is made up
  // of 4 interleaved rows within the 16 total rows. Insert the calls to this
  // function into the main loop for the highest call rate, or from a timer
  // interrupt
...
private:
  ...
  // Pin configuration helper methods (replacing macros)
  inline void lightRow_01_05_09_13() {
    digitalWrite(PIN_DMD_B, LOW);
    digitalWrite(PIN_DMD_A, LOW);
  }
  inline void lightRow_02_06_10_14() {
    digitalWrite(PIN_DMD_B, LOW);
    digitalWrite(PIN_DMD_A, HIGH);
  }
  inline void lightRow_03_07_11_15() {
    digitalWrite(PIN_DMD_B, HIGH);
    digitalWrite(PIN_DMD_A, LOW);
  }
  inline void lightRow_04_08_12_16() {
    digitalWrite(PIN_DMD_B, HIGH);
    digitalWrite(PIN_DMD_A, HIGH);
  }
  inline void latchShiftRegister() {
    digitalWrite(PIN_DMD_SCLK, HIGH);
    digitalWrite(PIN_DMD_SCLK, LOW);
  }
  inline void rowsOff() { digitalWrite(PIN_DMD_nOE, LOW); }
  inline void rowsOn() { digitalWrite(PIN_DMD_nOE, HIGH); }

  // Mirror of DMD pixels in RAM, ready to be clocked out by the main loop or
  // high speed timer calls
  byte *bDMDScreenRAM;
  ...
  // scanning pointer into bDMDScreenRAM, setup init @ 48 for the first valid
  // scan
  ...
  // Pin configuration member variables
  uint8_t PIN_DMD_A;
  uint8_t PIN_DMD_B;
  uint8_t PIN_DMD_SCLK;
  uint8_t PIN_DMD_nOE;
  uint8_t PIN_DMD_CLK;
  uint8_t PIN_DMD_R_DATA;
  uint8_t PIN_OTHER_SPI_nCS;
};
